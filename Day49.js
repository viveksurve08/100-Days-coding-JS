...
//? Write a funtion named isValidDate that takes a single string input representing a date determines if it is a valid date in the format MM/DD/YYYY. The valid date range should be from January 1, 1990, to December 31, 2099.
...
const isValidDate = (dateFormat) => {
  ...
  //? General Date Validity: This regular expression only check if the date string is a specific format and within plausible numerical ranges (e.g., 01-12 for months, 01-31 for days). It does not account for the varying number of days in different months. For example, it would consider "02/31/2020" as valid, though February never has 31 days.
