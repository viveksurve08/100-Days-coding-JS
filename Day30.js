...
// 2nd way- this way are complex 

const numberRangeRecursive = (a, b) => {
    if (typeof a !== "number" || typeof b !== "number" || a > b) {
      return [];
    }
    const generateRangeRecursive = (curr, end, arr) => {
      if (curr > end) {
        return arr;
      }
      arr.push(curr);
      return generateRangeRecursive(curr + 1, end, arr);
    };
    let rangeArr = [];
    return generateRangeRecursive(a, b, rangeArr);
  };
  
  console.log(numberRangeRecursive(0.5)); //Output: []
